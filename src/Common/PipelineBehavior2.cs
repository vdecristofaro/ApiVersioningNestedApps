...
namespace Common {
    public class PipelineBehavior2<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
        where TResponse : class {
        public async Task<TResponse> Handle( TRequest request, CancellationToken cancellationToken, RequestHandlerDelegate<TResponse> next ) {
            Console.WriteLine( $"{nameof( PipelineBehavior2<TRequest, TResponse> )} begin" );
            var response = await next();
            Console.WriteLine( $"{nameof( PipelineBehavior2<TRequest, TResponse> )} end" );
            return response;
        }
    }
}
